...
ResQ-Connect is a FastAPI-based backend that serves as the decision engine for emergency response systems. It leverages Google's Gemini generative AI to extract structured data from natural language emergency calls and intelligently routes incidents to the nearest available response units.
...
└──────────────────────────────────┘
        ↓
┌──────────────────────────────────┐
│   AI Engine (ai_engine.py)       │  ← Google Gemini integration
│   • Prompt engineering           │
│   • Response parsing             │
│   • Mock mode support            │
└──────────────────────────────────┘
...
| `ai_engine.py` | AI integration | Gemini API calls, prompt engineering, JSON parsing |
...
Set your `GOOGLE_API_KEY` in the environment. Optionally set `GEMINI_MODEL` to a specific model name (default: `models/gemini-1.0`):

```bash
export GOOGLE_API_KEY="ya29.your_api_key_here"
# Optional: override default model
export GEMINI_MODEL="models/gemini-1.0"
```
...
**Notes:** Install `requests` (used to call the Generative Language REST API) and ensure your project has access to the internet from the runtime environment.
...
│   ├── ai_engine.py      # Google Gemini integration and AI logic
...
- Managed generative models via Google Generative Language API
- API key based access or service account tokens
- Pay-per-use pricing and enterprise support
- Configurable model selection via the `GEMINI_MODEL` env var
...
Built with Google Gemini, FastAPI, and a focus on saving lives through intelligent automation.
